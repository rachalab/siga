...
class DatabaseBackendTest extends KernelTestBase {
   ...
   */
  public function testAvoidDuplicatesFalse() {
   ...
   */
  public function testAvoidDuplicatesTrue() {
    ...
    $job = Job::create('avoid_duplicates', ['test' => '1']);
    $this->expectException(DuplicateJobException::class, "A duplicate job was rejected.");
    $this->firstQueue->enqueueJob($job);
   ...
   */
  protected function assertQueuedCount($expected_count, QueueInterface $queue, $message = NULL) {
    if (empty($message)) {
