...
namespace Drupal\triples_field\Plugin\Field\FieldType;
...
/**
 * Plugin implementation of the triple field type.
 *
 * @FieldType(
 *   id = "triples_field",
 *   label = @Translation("Triple Field"),
 *   description = @Translation("Triple Field."),
 *   default_widget = "triples_field_table",
 *   default_formatter = "triples_field_table"
 * )
 */
class TriplesField extends FieldItemBase {
  ...
  public function storageSettingsForm(array &$form, FormStateInterface $form_state, $has_data): array {
    ...
    foreach ($subfields as $subfield) {
      ...
      $element['storage'][$subfield]['maxlength'] = [
        ...
        '#min' => 1,
        '#states' => [
          'visible' => [":input[name*='[storage][$subfield][type]']" => ['value' => 'string']],
        ],
      ];
      ...
      $element['storage'][$subfield]['precision'] = [
        ...
        '#max' => 32,
        '#states' => [
          'visible' => [":input[name*='[storage][$subfield][type]']" => ['value' => 'numeric']],
        ],
      ];
      ...
      $element['storage'][$subfield]['datetime_type'] = [
        ...
        '#states' => [
          'visible' => [":input[name*='[storage][$subfield][type]']" => ['value' => 'datetime_iso8601']],
        ],
      ];
    ...
    foreach ($subfields as $subfield) {
      ...
      if (static::isListAllowed($type)) {
        ...
        $element[$subfield]['allowed_values'] = [
          ...
          // @see: TriplesField::validateAllowedValues()
          '#storage_type' => $type,
  ...
  public function createDate(string $subfield): ?DateTimePlus {
    ...
    if ($this->{$subfield}) {
      ...
      $format = static::DATETIME_DATETIME_STORAGE_FORMAT;
      if ($is_date_only) {
        $format = static::DATETIME_DATE_STORAGE_FORMAT;
        $this->{$subfield} = date($format, strtotime($this->{$subfield}));
      }
